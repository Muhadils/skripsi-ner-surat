...
class WeightedTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        weights = torch.tensor([1.0, 15.0, 15.0, 15.0, 15.0]).to(logits.device)
...
def main():
    print("--- MEMULAI PELATIHAN MODEL TERPISAH (LUAS & HARGA) ---")
    with open('dataset_ner.json', 'r', encoding='utf-8') as f:
        raw_data = json.load(f)
    with_ent = [d for d in raw_data if any(e['label'] in ['LUAS', 'HARGA'] for e in d['entities'])]
    without_ent = [d for d in raw_data if not any(e['label'] in ['LUAS', 'HARGA'] for e in d['entities'])]
    ...
    print("SELESAI! Model LUAS & HARGA disimpan di: ./model_ner_luas_harga")
