        ...
        for ent in example_entities:
            ...
            for idx, (t_start, t_end) in enumerate(offset_mapping):
                ...
                if t_start < end and t_end > start:
                    if t_start == start: label_ids[idx] = label2id["B-PERSON"]
                    else: label_ids[idx] = label2id["I-PERSON"]
...
def main():
    ...
    with_ent = [d for d in raw_data if any(e['label'] == 'PERSON' for e in d['entities'])]
    without_ent = [d for d in raw_data if not any(e['label'] == 'PERSON' for e in d['entities'])]
    ...
    trainer.train()
    model.save_pretrained("./model_ner_person")
    tokenizer.save_pretrained("./model_ner_person")
    print("SELESAI! Model PERSON disimpan.")
