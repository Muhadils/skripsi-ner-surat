...
def tokenize_and_align_labels(examples):
    ...
    for i, example_entities in enumerate(examples["entities"]):
        ...
        for ent in example_entities:
            if ent["label"] not in ["ALAMAT", "KERJA", "TGL", "JABATAN"]: continue
...
def main():
    ...
    target_labels = ["ALAMAT", "KERJA", "TGL", "JABATAN"]
    with_ent = [d for d in raw_data if any(e['label'] in target_labels for e in d['entities'])]
    without_ent = [d for d in raw_data if not any(e['label'] in target_labels for e in d['entities'])]
